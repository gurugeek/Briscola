...
*
*/
getOpponentCard = function(nickname){
  var cards = document.getElementsByClassName("computer");
  ...
  $.ajax(
  {
      ...
      success: function( data )
      {
          ...
          if(data.index !== false){
            ...
            setTimeout(getOpponentCard(nickname),5000);
          }
...
multiplayer = function(playCardClass, index){
  var spaceToThrowCard = document.getElementsByClassName("firstEmptySpace");

  var cards = document.getElementsByClassName(playCardClass);

  if(index < 0){
    getOpponentCard(localStorage.user);
    ...
    spaceToThrowCard[0].name = briscolaPlay.mineCards[cardPlaceIndex];//no of card in deck
    ...
    setTimeout(getOpponentCard(localStorage.user),5000);
  }
...
@param index - the index of the card to be played.
      Valid indexes are 0-2 as 3 card are held by players at each time.
...
playTurn = function(playCardClass = "computer", index = -1){
  ...
  if(mode === "4"){
    multiplayer(playCardClass,index);
    return;
  }
}
...
roundOverFunction = function(){
  ...
  if(briscolaPlay.result[briscolaPlay.myId] < briscolaPlay.result[briscolaPlay.hisId]){
    name = localStorage.opponent;
  }
  if(briscolaPlay.result[briscolaPlay.myId] == briscolaPlay.result[briscolaPlay.hisId]){
    name = "Draw.";
  }
  var tmp = localStorage.user;
  localStorage.clear();
...
function BriscolaPlay(){
  ...
  if(localStorage['shuffeledCards'] !== undefined){
    this.mixedCards = JSON.parse(localStorage['shuffeledCards']);
    console.log(this.mixedCards);
  }
...
function setMultiplayer(){
  if(localStorage['first'] === "0"){
    console.log("mode "+localStorage.first);
    temp = briscolaPlay.mineCards;
    briscolaPlay.mineCards = briscolaPlay.hisCards;
    briscolaPlay.hisCards = temp;
    briscolaPlay.onTurn = false;
    briscolaPlay.myTurn = 1;//who firstplayed 0 if the player, 1 if opponent
...
localStorage.setItem("playMode","4");//random mode
////////////////////////////////////////////////////////////
//do after ready
